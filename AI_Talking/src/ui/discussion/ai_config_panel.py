...
class AIConfigPanel(QWidget):
    ...
    def _create_ai_config_box(
        self, title: str, default_api: str, ai_id: str
    ) -> QContainerWidget:
        """创建AI配置框
        ...
        # 根据不同AI名称设置不同的样式，边框色与背景色相同
        if ai_id == "ai1":
            # 学者AI1：与ai1-message样式匹配
            background_color = "#e3f2fd"
            border_color = background_color  # 边框色与背景色相同
        elif ai_id == "ai2":
            # 学者AI2：与ai2-message样式匹配
            background_color = "#f3e5f5"
            border_color = background_color  # 边框色与背景色相同
        elif ai_id == "ai3":
            # 专家AI3：与ai3-message样式匹配
    ...
    def update_model_list(self, api_combo, model_combo):
        ...
        logger.info(f"更新讨论模型列表，当前API: {api}")
    ...
    def _on_models_loaded(self, model_combo, api, models):
        ...
        if model_combo.count() > 0:
            # 查找并选择默认模型
            default_model = self._get_default_model(model_combo, api)
    ...
    def _get_default_model(self, model_combo, api: str) -> str:
        """
        根据AI类型获取默认模型
        ...
        """
        # 根据模型组合框对象确定AI类型
        if model_combo == self.model1_combo:
            # 学者AI1默认模型
            return "gpt-oss:120b-cloud" if api == "ollama" else "gpt-3.5-turbo"
        elif model_combo == self.model2_combo:
            # 学者AI2默认模型
            return "cogito-2.1:671b-cloud" if api == "ollama" else "gpt-3.5-turbo"
        elif model_combo == self.model3_combo:
            # 专家AI3默认模型
            return "deepseek-v3.1:671b-cloud" if api == "ollama" else "gpt-4"
        return ""
    ...
    def reinit_ui(self):
        ...
        # 更新AI1配置
        self.ai1_title_label.setText(i18n.translate("scholar_ai1"))
        self.ai1_model_label.setText(i18n.translate("model") + ":")
        self.ai1_api_provider_label.setText(i18n.translate("model_provider") + ":")

        # 更新AI2配置
        self.ai2_title_label.setText(i18n.translate("scholar_ai2"))
        self.ai2_model_label.setText(i18n.translate("model") + ":")
        self.ai2_api_provider_label.setText(i18n.translate("model_provider") + ":")

        # 更新AI3配置
        self.ai3_title_label.setText(i18n.translate("expert_ai3"))
        self.ai3_model_label.setText(i18n.translate("model") + ":")
        self.ai3_api_provider_label.setText(i18n.translate("model_provider") + ":")

        # 更新模型加载提示
        if self.model1_combo.count() > 0 and self.model1_combo.itemText(
            0
        ) == i18n.translate("loading"):
            self.model1_combo.setItemText(0, i18n.translate("loading"))
        if self.model2_combo.count() > 0 and self.model2_combo.itemText(
            0
        ) == i18n.translate("loading"):
            self.model2_combo.setItemText(0, i18n.translate("loading"))
        if self.model3_combo.count() > 0 and self.model3_combo.itemText(
            0
        ) == i18n.translate("loading"):
            self.model3_combo.setItemText(0, i18n.translate("loading"))
