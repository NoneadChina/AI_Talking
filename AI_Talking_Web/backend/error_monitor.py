...
class ErrorMonitor:
    ...
    def _initialize(self):
        ...
        # API错误统计：{api_endpoint: {error_type: count}}
        self.api_error_counts = defaultdict(lambda: defaultdict(int))
    ...
    def record_error(self, error_type: str, error_message: str, module: str = "unknown", api_endpoint: str = ""):
        """
        记录错误信息
        ...
        with self._lock:
            ...
            # 如果提供了API端点，增加API错误计数
            if api_endpoint:
                self.api_error_counts[api_endpoint][error_type] += 1
            
            # 记录日志
            logger.error(f"[{module}] {error_type}: {error_message} (API: {api_endpoint})")
    ...
    def get_error_rate(self, window_seconds: int = 60) -> float:
        ...
        with self._lock:
            current_time = time.time()
            recent_errors = [t for t in self.error_window if current_time - t < window_seconds]
            return len(recent_errors) / window_seconds if window_seconds > 0 else 0
    
    def get_api_error_counts(self) -> dict:
        """
        获取API错误统计
        ...
        """
        with self._lock:
            # 将defaultdict转换为普通dict，方便序列化
            return {endpoint: dict(counts) for endpoint, counts in self.api_error_counts.items()}
