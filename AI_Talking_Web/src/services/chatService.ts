import { Message, ChatConfig, ChatMessageDisplay } from '../types/types';
...
class ChatService {
    ...
    async startChat(
        config: ChatConfig,
        onMessage: (message: ChatMessageDisplay) => void,
        ...
        try {
            ...
            for (let round = 1; round <= config.rounds; round++) {
                ...
                let ai1CurrentMessages: Message[] = [...ai1Messages];
                if (round === 1) {
                    // 第一轮：AI1使用包含主题的初始消息
                ...
                const ai1Response = await this.aiService.getAIResponse(
                    config.api1,
                    config.model1,
                    ai1CurrentMessages,
                    config.temperature
                ...
                // 显示AI1的回复
                const ai1Message: ChatMessageDisplay = {
                ...
                let ai2CurrentMessages: Message[] = [{ role: 'system', content: fullAi2SystemPrompt }];
                ...
                const ai2Response = await this.aiService.getAIResponse(
                    config.api2,
                    config.model2,
                    ai2CurrentMessages,
                    config.temperature
                ...
                // 显示AI2的回复
                const ai2Message: ChatMessageDisplay = {
     ...
     * @returns 是否继续对话
     */
    private checkTimeLimit(onStatusUpdate: (status: string) => void, showCurrentTime: boolean): boolean {
    ...
    getChatHistory(): ChatMessageDisplay[] {
