...
import { ApiType, ApiSettings, ChatConfig, ChatMessageDisplay } from './types/types';
...
class App {
    ...
    private isDebateRunning: boolean = false;
    private chatHistory: ChatMessageDisplay[] = [];
    ...
    private async loadSettings(): Promise<void> {
        try {
            ...
            if (settings) {
                ...
                const deepseekBaseUrlInput = document.getElementById('deepseekBaseUrl') as HTMLInputElement;
                const ollamaUrlInput = document.getElementById('ollamaUrl') as HTMLInputElement;
                const ollamaKeyInput = document.getElementById('ollamaKey') as HTMLInputElement;
                ...
                const discussionSystemPromptInput = document.getElementById('discussionSystemPrompt') as HTMLTextAreaElement;
                const discussionAI1SystemPromptInput = document.getElementById('discussionAI1SystemPrompt') as HTMLTextAreaElement;
                const discussionAI2SystemPromptInput = document.getElementById('discussionAI2SystemPrompt') as HTMLTextAreaElement;
                const debateSystemPromptInput = document.getElementById('debateSystemPrompt') as HTMLTextAreaElement;
                const debateAI1SystemPromptInput = document.getElementById('debateAI1SystemPrompt') as HTMLTextAreaElement;
                const debateAI2SystemPromptInput = document.getElementById('debateAI2SystemPrompt') as HTMLTextAreaElement;
        ...
        const api1Select = document.getElementById('api1') as HTMLSelectElement;
        const model1Select = document.getElementById('model1') as HTMLSelectElement;
        const api2Select = document.getElementById('api2') as HTMLSelectElement;
        ...
        const debateApi1Select = document.getElementById('debate-api1') as HTMLSelectElement;
        const debateModel1Select = document.getElementById('debate-model1') as HTMLSelectElement;
        const debateApi2Select = document.getElementById('debate-api2') as HTMLSelectElement;
        const debateModel2Select = document.getElementById('debate-model2') as HTMLSelectElement;
        ...
        // 加载系统提示词
        const commonSystemPromptInput = document.getElementById('commonSystemPrompt') as HTMLTextAreaElement;
        const ai1SystemPromptInput = document.getElementById('ai1SystemPrompt') as HTMLTextAreaElement;
        const ai2SystemPromptInput = document.getElementById('ai2SystemPrompt') as HTMLTextAreaElement;
     ...
     * 获取并显示Ollama模型清单
     */
    private async fetchAndDisplayOllamaModels(): Promise<void> {
        try {
    ...
    private async stopDebate(): Promise<void> {
        this.isDebateRunning = false;
        this.enableUI(true);
        this.updateStatus('辩论已停止');
    }

    private async sendHumanMessage(): Promise<void> {
        const messageInput = document.getElementById('human-message') as HTMLInputElement;
        ...
        // 获取聊天配置
        const providerSelect = document.getElementById('human-provider') as HTMLSelectElement;
        const modelSelect = document.getElementById('human-model') as HTMLSelectElement;
        const temperatureInput = document.getElementById('human-temperature') as HTMLInputElement;
        ...
        try {
            ...
            // 使用流式请求
            await this.apiService.sendChatMessageStream(
                config,
                // 收到新数据时的回调
                (chunk: string) => {
    ...
    private getChatConfig(): ChatConfig | null {
        const topicInput = document.getElementById('topic') as HTMLInputElement;
        const api1Select = document.getElementById('api1') as HTMLSelectElement;
        const model1Select = document.getElementById('model1') as HTMLSelectElement;
        const api2Select = document.getElementById('api2') as HTMLSelectElement;
        ...
        return {
            ...
            api1: api1Select.value as ApiType,
            model2: model2Select.value,
            api2: api2Select.value as ApiType,
    ...
    private getDebateConfig(): ChatConfig | null {
        const topicInput = document.getElementById('debate-topic') as HTMLInputElement;
        const api1Select = document.getElementById('debate-api1') as HTMLSelectElement;
        const model1Select = document.getElementById('debate-model1') as HTMLSelectElement;
        const api2Select = document.getElementById('debate-api2') as HTMLSelectElement;
        const model2Select = document.getElementById('debate-model2') as HTMLSelectElement;
        const roundsInput = document.getElementById('debate-rounds') as HTMLInputElement;
        const timeLimitInput = document.getElementById('debate-timeLimit') as HTMLInputElement;
        const temperatureInput = document.getElementById('debate-temperature') as HTMLInputElement;
    ...
    private async appendToChatHistory(message: ChatMessageDisplay): Promise<void> {
        // 添加到聊天历史数组
    ...
    private clearHumanChat(): void {
        this.clearChatHistory();
    }

    private enableUI(enabled: boolean): void {
        // 启用/禁用讨论相关按钮
        const startButton = document.getElementById('startChat') as HTMLButtonElement;
        const stopButton = document.getElementById('stopChat') as HTMLButtonElement;
        ...
        // 启用/禁用辩论相关按钮
        const startDebateButton = document.getElementById('startDebate') as HTMLButtonElement;
        const stopDebateButton = document.getElementById('stopDebate') as HTMLButtonElement;
        
        if (startDebateButton) startDebateButton.disabled = !enabled;
        if (stopDebateButton) stopDebateButton.disabled = enabled;
    }
    ...
    private updateModelList(modelSelectId: string): void {
        // 根据API类型更新模型列表
        const modelSelect = document.getElementById(modelSelectId) as HTMLSelectElement;
        const apiSelect = document.getElementById(modelSelectId.replace('model', 'api')) as HTMLSelectElement;
    ...
    private async viewHistory(index: number): Promise<void> {
        try {
    ...
    private async deleteHistory(index: number): Promise<void> {
    ...
    private async deleteSelectedHistory(): Promise<void> {
        // 获取所有选中的历史记录
        const checkboxes = document.querySelectorAll('input[type="checkbox"][data-index]:checked');
        const indices = Array.from(checkboxes).map(cb => parseInt((cb as HTMLInputElement).dataset.index!));
        if (indices.length === 0) return;
        
        if (!confirm(`确定要删除选中的 ${indices.length} 条历史记录吗？`)) return;
    ...
    private selectAllHistory(e: Event): void {
    ...
    private async handleLogin(e: Event): Promise<void> {
        ...
        const errorElement = document.getElementById('loginError') as HTMLDivElement;
